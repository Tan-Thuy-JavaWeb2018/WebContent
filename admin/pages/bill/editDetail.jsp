...
<%@ page import="Objects.BillDetailsFull"%>
<!DOCTYPE html>
...
<body>
	<% BillDetailsFull billDetail = (BillDetailsFull) request.getAttribute("billDetail"); %>
							...
							<div class="card-body card-block">
								<form action="editdetail" method="post"
									...
									<input type="hidden" class="form-control"
										value="<%=billDetail.getId()%>" name="id">
									<input type="hidden" class="form-control"
										value="<%=billDetail.getId_hoadon()%>" name="idHoaDon">
									<div class="row">
										<div class="col-md-6">
											<div class="form-group">
												<label class="form-control-label">Chọn sản phẩm</label> <select
													...
													<%
														for (Products product : (ArrayList<Products>) request.getAttribute("product")) {
															if(product.getId() == billDetail.getId_sanpham()){
													%>
													<option value="<%=product.getId()%>" selected="selected"><%=product.getTensanpham()%></option>
													<%
														} else {
													%>
													<option value="<%=product.getId()%>"><%=product.getTensanpham()%></option>
													<%} }%>
											...
											<div class="form-group">
												<label class="form-control-label">Giá bán</label> <input
													...
													id="gia" value="<%=billDetail.getGiagoc()%>">
											</div>
											<div class="form-actions form-group">
												<button type="submit" class="btn btn-success">Lưu
													lại</button>
												<a class="btn btn-danger" href="viewdetail?idHoaDon=<%=billDetail.getId_hoadon()%>">Hủy bỏ</a>
											</div>
										</div>
										...
										<div class="col-md-6">
											<div class="form-group">
												<label class="form-control-label">Số lượng</label> <input
													...
													value="<%=billDetail.getSoluong()%>">
											</div>
											<div class="form-group">
												<label class="form-control-label">Khuyến mãi</label> <input
													type="number" class="form-control" readonly="readonly"
													id="khuyenmai" value="<%=billDetail.getKhuyenmai() %>">
											</div>
